...
public abstract class BaseDao<T> {
	 ...
	 * 保存
	 * 
	 * @param entity 在做单元测试时，要手动提交事务，否则事物会回滚,开启事务，保存成功后关闭连接
	 */
...
//		session.close();
	...
	/**
	 * @author XieXing
	 * @createDate 2019年5月4日 下午3:32:14
	 * @description 非单元测试直接保存操作
	 * @param entity
	 */
...
//		session.save(entity);
//		session.close();
		getSession().save(entity);//单元测试失败时，改成上面的代码
	}

	/**
	 * @author XieXing
	 * @createDate 2019年5月4日 上午11:03:19
	 * @description 批量保存
	 * @param list
	 * @return
	 */
	public int batchSave(final List<T> list) {
		for (int i = 0; i < list.size(); i++) {
			save(list.get(i));
			if (i % 100 == 0) {// BATCH_MAX_ROW,设置一个批量的行数,当完成一个批量时，手动清除缓存
				getSession().flush();
				getSession().clear();
			}
		}
	...
	public Page<T> getPageList(Map<String, Object> params, Map<String, String> orderOrGroupBy) {
		...
		if (orderOrGroupBy != null && orderOrGroupBy.size() != 0) {
			for (Entry<String, String> en : orderOrGroupBy.entrySet()) {
				// orderOrGroupBy中key为hql语句的group by 或者 order by （格式orderOrGroupBy.put("group
				// by","id");）
			...
			for (Entry<String, Object> en : params.entrySet()) {
				...
				if (!"page".equals(en.getKey()) && !"rows".equals(en.getKey())) {
					if (en.getKey().contains("?")) {
						// 带有?的hql语句（格式：params.put("id = ?","12");）方法：query.setParameter(int arg0,
						// Object arg1)
						...
						// 带有in的hql语句（格式：params.put("id in
						// (:id)",{2,3});）方法：query.setParameterList(String arg0, Object[] arg1)
						String key = en.getKey();
						query1.setParameterList(key.substring(key.indexOf("(") + 2, key.indexOf(")")).trim(),
								(Object[]) en.getValue());
						query2.setParameterList(key.substring(key.indexOf("(") + 2, key.indexOf(")")).trim(),
								(Object[]) en.getValue());
					} else if (en.getKey().contains("(") && en.getKey().contains(")") && en.getKey().contains(":")) {
						/*
						 * // 特殊处理带有OR的模糊查询 (code LIke :searchStr AND cnName LIKE :searchStr OR
						 * shortName LIKE :searchStr) String key = en.getKey().trim(); int beginIndex =
						 * key.indexOf(":") + 1; int endIndex = key.indexOf(" ", beginIndex);
						 */
	...
	/**
	 * @author XieXing
	 * @createDate 2019年5月6日 下午6:25:24
	 * @description 条件查询获取数据数量，必须带一个条件
	 * @param params
	 * @return
	 */
	public Integer getCount(Map<String, Object> params) {
		StringBuffer sb2 = new StringBuffer("SELECT COUNT(*) FROM " + className);
		// 如果限制条件不为空，就拼接参数
		if (params != null && params.size() != 0 && params.size() > 0) {
			sb2.append(" WHERE");
			for (Entry<String, Object> en : params.entrySet()) {
				// params中key为hql语句的where条件,value为条件注入的参数（格式：params.put("id = ?","12");）
				// 跳过page和rows的拼接，需要单独拼接
	...
	/**
	 * 根据某个参数获取实体，该参数只能是和主键一样，是唯一的值，其参数只能一个（格式：query.setParameter(String arg0,
	 * Object arg1)）
	 * 
	 * @param params
	 * @return
	...
	public List<T> getByIds(Integer[] ids) {
		List<T> list = getSession().createQuery("FROM " + className + " WHERE id IN (:ids)")
				.setParameterList("ids", ids).list();
		return list;
	}

	/**
	 * 获取不分页的数据，不分页page和rows设置为0就行，可以设置 是and 还是or or的在and之间用括号包住 例如: and (。。。or。。。)
	 * and
	 * 
	 * @param params
	 * @param orderOrGroupBy
	...
	public List<T> getLikeList(Map<String, Object> params, Map<String, String> orderOrGroupBy) {
		...
		if (params != null && params.size() != 0) {
			...
			for (Entry<String, Object> en : params.entrySet()) {
				...
				if (!"page".equals(en.getKey()) && !"rows".equals(en.getKey())) {
					if (en.getKey().contains("?")) {
						// 带有?的hql语句（格式：params.put("and id = ?","12");）方法：query.setParameter(int arg0,
						// Object arg1)
						query1.setParameter(index, en.getValue());
						query2.setParameter(index, en.getValue());
						index++;
					} else if ((en.getKey().toUpperCase()).contains(" IN")) {
						// 带有in的hql语句（格式：params.put("and id in
						// (:id)",{2,3});）方法：query.setParameterList(String arg0, Object[] arg1)
						String key = en.getKey();
						query1.setParameterList(key.substring(key.indexOf("(") + 2, key.indexOf(")")).trim(),
								(Object[]) en.getValue());
	...
	/**
	 * 获取分页的数据，不分页page和rows设置为0就行，可以设置 是and 还是or or的在and之间用括号包住 例如: and (。。。or。。。)
	 * and
	 * 
	 * @param params
	 * @param orderOrGroupBy
