...
/**
 * @author lch770
 * @date 2018/01/23 03:36:41
 *
 */
public class ExcelUtil {
	
	private Workbook workbook;
	public static final String pattern = "yyyy-MM-dd hh:mm:ss";// 日期格式
	 ...
	 * @throws Exception
	 */
	public ExcelUtil(String filePath) throws IOException, Exception {
		if(isExcel(filePath)) {
			...
			throw new Exception("你指定的文件不是一个标准的excel文件");
		}
	}
	 ...
	 * 
	 */
	public ExcelUtil write(List<List<String>> rowData) throws IOException {
	 ...
	 * @throws IOException
	 */
	public ExcelUtil write(List<List<String>> rowData, String sheetName, boolean isNewSheet) throws IOException {
		Sheet sheet = null;
		if (isNewSheet) {
	 ...
	 * @throws IOException
	 */
	public ExcelUtil write(int sheetIx, List<List<String>> rowData, boolean isAppend) throws IOException {
		if (isAppend) {
	 ...
	 * @throws IOException
	 */
	public ExcelUtil write(int sheetIx, List<List<String>> rowData, int startRow) throws IOException {
		Sheet sheet = workbook.getSheetAt(sheetIx);
		...
		for (int i = 0; i < dataSize; i++) {
			...
			for (int j = 0; j < rowData.get(i).size(); j++) {
				...
				cell.setCellValue(rowData.get(i).get(j).equals("null")?"":rowData.get(i).get(j));
			}
		}
		return this;
	}
	...
	/**
	 * @author xiexing
	 * @param achievementNeeds
	 */
	public void write2(List<AchievementNeed> achievementNeeds) {
