...
function drawTable(){
		...
		$("#table").jqGrid({
			url:path+"/grade/getList?content="+content,
			datatype: 'json',
			...
			colNames: ['年级名称','备注'],
			colModel: [
				{name: 'gradeName', index: 'gradeName', align: 'center'},
				{name: 'gradeRemark', index: 'gradeRemark', align: 'center'},
			],
			rowNum: 10,		//每页显示多少条
			...
			ondblClickRow:function(id){
				window.location.href="${pageContext.request.contextPath}/grade/toAdd?id="+id+"&operate=update";
			}
	  	  });
		};
	...
	.navButtonAdd('#pager', {
		...
		onClickButton: function(){//按钮点击函数
	       window.location.href="${pageContext.request.contextPath}/grade/toAdd?operate=add";
		}
	}).navButtonAdd('#pager', {
		caption: '修改',
		...
		onClickButton: function(){//按钮点击函数
			...
			window.location.href="${pageContext.request.contextPath}/grade/toAdd?id="+id+"&operate=show";
		}
	})  .navButtonAdd('#pager', {
		caption: '删除',
	  ...
	  function updatePagerIcons(table) {
		...
		$("#searchBtn").click(function(){
			 $("#table").jqGrid("setGridParam", {
	                   url: "${pageContext.request.contextPath}/grade/getList",
	                   mtype: "post",
	                   dataType: "json",
	                   postData: {	                	   	             
	                	   start: $("#test3").val(),
