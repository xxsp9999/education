...
function drawTable(){
		...
		$("#table").jqGrid({
			url:path+"/studentpractice/getList?content="+content,
			datatype: 'json',
			...
			width: jqTablew,
			height: 350,
			colNames: ['学号','姓名','实践事项','获奖情况','操评','实践时间','审核状态','备注'],
			colModel: [
				{name: 'practiceStudent.stuNumber', index: '', align: 'center'},
				{name: 'practiceStudent.stuName', index: '', align: 'center'},
				{name: 'itemName', index: '', align: 'center'},
				{name: 'prizeSituation', index: '', align: 'center'},
				{name: 'practiceScore', index: '', align: 'center'},
				{name: 'praceticeDate', index: '', align: 'center'},
				{name: 'checkState', index: '', align: 'center'},
				{name: 'practiceRemark', index: '', align: 'center'},
			],
			rowNum: 10,		//每页显示多少条
			...
			ondblClickRow:function(id){
				window.location.href="${pageContext.request.contextPath}/studentpractice/toAdd?id="+id+"&operate=update";
			}
	  	  });
		};
	...
	.navButtonAdd('#pager', {
		...
		onClickButton: function(){//按钮点击函数
	       window.location.href="${pageContext.request.contextPath}/studentpractice/toAdd?operate=add";
		}
	}).navButtonAdd('#pager', {
		caption: '修改',
	  ...
	  function updatePagerIcons(table) {
		...
		$("#searchBtn").click(function(){
			 $("#table").jqGrid("setGridParam", {
	                   url: "${pageContext.request.contextPath}/studentpractice/getList",
	                   mtype: "post",
	                   dataType: "json",
	                   postData: {	                	   	             
	                	   start: $("#test3").val(),
